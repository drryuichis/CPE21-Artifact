...
#define INNER_STENCIL_MEMCPY_ASYNC(R0, R1, R2, R3, R4, R5, R6, R7, R8) i++; if (i >= x4) { return; } \
    zrs##R8 = u[IDX3(i+N_RADIUS,j,k)]; \
...
#define PML_STENCIL_MEMCPY_ASYNC(R0, R1, R2, R3, R4, R5, R6, R7, R8) i++; if (i >= x4) { return; } \
    zrs##R8 = u[IDX3(i+N_RADIUS,j,k)]; \
    __pipeline_wait_prior(0); \
    __syncthreads(); \
    if (i+1 < x4) { \
...
__global__ void __launch_bounds__(1024) kernel_7r_25d_inner(
    ...
    while (true) {
        INNER_STENCIL_MEMCPY_ASYNC(0, 1, 2, 3, 4, 5, 6, 7, 8);
        INNER_STENCIL_MEMCPY_ASYNC(1, 2, 3, 4, 5, 6, 7, 8, 0);
        INNER_STENCIL_MEMCPY_ASYNC(2, 3, 4, 5, 6, 7, 8, 0, 1);
        INNER_STENCIL_MEMCPY_ASYNC(3, 4, 5, 6, 7, 8, 0, 1, 2);
        INNER_STENCIL_MEMCPY_ASYNC(4, 5, 6, 7, 8, 0, 1, 2, 3);
        INNER_STENCIL_MEMCPY_ASYNC(5, 6, 7, 8, 0, 1, 2, 3, 4);
        INNER_STENCIL_MEMCPY_ASYNC(6, 7, 8, 0, 1, 2, 3, 4, 5);
        INNER_STENCIL_MEMCPY_ASYNC(7, 8, 0, 1, 2, 3, 4, 5, 6);
        INNER_STENCIL_MEMCPY_ASYNC(8, 0, 1, 2, 3, 4, 5, 6, 7);
    }
...
__global__ void __launch_bounds__(1024) kernel_7r_25d_pml(
    ...
    while (true) {
        PML_STENCIL_MEMCPY_ASYNC(0, 1, 2, 3, 4, 5, 6, 7, 8);
        PML_STENCIL_MEMCPY_ASYNC(1, 2, 3, 4, 5, 6, 7, 8, 0);
        PML_STENCIL_MEMCPY_ASYNC(2, 3, 4, 5, 6, 7, 8, 0, 1);
        PML_STENCIL_MEMCPY_ASYNC(3, 4, 5, 6, 7, 8, 0, 1, 2);
        PML_STENCIL_MEMCPY_ASYNC(4, 5, 6, 7, 8, 0, 1, 2, 3);
        PML_STENCIL_MEMCPY_ASYNC(5, 6, 7, 8, 0, 1, 2, 3, 4);
        PML_STENCIL_MEMCPY_ASYNC(6, 7, 8, 0, 1, 2, 3, 4, 5);
        PML_STENCIL_MEMCPY_ASYNC(7, 8, 0, 1, 2, 3, 4, 5, 6);
        PML_STENCIL_MEMCPY_ASYNC(8, 0, 1, 2, 3, 4, 5, 6, 7);
    }
