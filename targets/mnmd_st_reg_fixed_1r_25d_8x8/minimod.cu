...
#define STENCIL(R0, R1, R2, R3, R4, R5, R6, R7, R8) i++; if (i >= nx) { return; } \
...
__global__ void __launch_bounds__(1024) kernel_1r_25d(
    ...
    while (true) {
        ...
        STENCIL(1, 2, 3, 4, 5, 6, 7, 8, 0);
        STENCIL(2, 3, 4, 5, 6, 7, 8, 0, 1);
        STENCIL(3, 4, 5, 6, 7, 8, 0, 1, 2);
        STENCIL(4, 5, 6, 7, 8, 0, 1, 2, 3);
        STENCIL(5, 6, 7, 8, 0, 1, 2, 3, 4);
        STENCIL(6, 7, 8, 0, 1, 2, 3, 4, 5);
        STENCIL(7, 8, 0, 1, 2, 3, 4, 5, 6);
        STENCIL(8, 0, 1, 2, 3, 4, 5, 6, 7);
    }
}
