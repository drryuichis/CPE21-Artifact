...
double calculate_sd(double data[], int count) {
...
int main(int argc, char *argv[])
{
    ...
    double time_modeling_all[niters];
    ...
    for (uint iiter = 0; iiter < (disable_warm_up_iter ? niters : niters+1); iiter++) {
        ...
        if (!warm_up_iter) {
            int i = disable_warm_up_iter ? iiter : iiter - 1;
            time_modeling_all[i] = (end_m.tv_sec  - start_m.tv_sec) +
    ...
    for (int i = 0; i < niters; i++) {
        time_modeling += time_modeling_all[i];
        printf("Time modeling (interation %d): %g\n", i, time_modeling_all[i]);
    }

    printf("Time modeling (average): %g s\n", time_modeling / niters);
    printf("Time modeling (stanard deviation): %g\n", calculate_sd(time_modeling_all, niters));
}
